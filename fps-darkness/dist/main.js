...
function isWallRect(x, y, rad) {
    return (isWall(x - rad, y - rad) ||
        isWall(x + rad, y - rad) ||
        isWall(x - rad, y + rad) ||
        isWall(x + rad, y + rad));
}
...
function spawnItem(x, y) {
    if (Math.random() < 0.3) {
        const types = ["battery", "medkit", "ammo"];
        items.push({
            x,
            y,
...
function moveWithCollision(x, y, nx, ny, rad) {
    let fx = nx;
    let fy = ny;
    if (isWallRect(fx, y, rad))
        fx = x;
    if (isWallRect(x, fy, rad))
        fy = y;
    return { x: fx, y: fy };
}
// ---- Update ----
function update() {
    ...
    for (const e of enemies) {
        ...
        // Visibility check
        const inFlashlight = player.flashlightOn &&
...
function render() {
    ...
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
            ...
            if (sx + CELL < 0 || sx > screenW || sy + CELL < 0 || sy > screenH)
                continue;
            if (map[r][c] === 1) {
    ...
    for (const it of items) {
        const sx = it.x - camX;
        const sy = it.y - camY;
        const glow = it.type === "battery" ? "#0f0" : it.type === "medkit" ? "#f0f" : "#0ff";
        ctx.beginPath();
...
function renderFlashlight(cx, cy) {
    if (!player.flashlightOn) {
